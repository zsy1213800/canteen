    ...
    <div class="card" style="margin-bottom: 10px">
      ...
      <el-table :data="data.tableData" style="width: 100%">
        ...
        <el-table-column prop="license" label="营业执照" >
          <template #default="scope">
            <img v-if="scope.row.license" :src="scope.row.license" alt="" style="width: 50px; height: 50px; border-radius: 50%" />
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="data.formVisible" title="信息" width="40%" destroy-on-close>
      <el-form :model="data.form" label-width="100px" style="padding-right: 50px">
        ...
        <el-form-item label="密码">
          <el-input v-model="data.form.password" autocomplete="off" :disabled="!!data.form.id" show-password/>
        </el-form-item>
        ...
        <el-form-item label="营业执照">
          <el-upload action="http://localhost:9090/files/upload" :on-success="licenseFileUpload">
            <el-button type="primary">点击上传</el-button>
          </el-upload>
        </el-form-item>
...
const save = () => {
  request.request({
    ...
    url: data.form.id ? '/business/update' : '/business/add',
    data: data.form
  }).then(res => {
    if (res.code === '200') {  //成功
...
const licenseFileUpload = (file) => {
  data.form.license = file.data
}
